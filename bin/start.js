...
var initialLogger = function (config) {
	...
	log4js.configure({
		appenders: { 
			...
			info: { type: 'file', filename: infoPath, "maxLogSize": 10485760, "backups": 365 },
			exception: { type: 'file', filename: exceptionPath, "maxLogSize": 10485760, "backups": 10 },
			threat: { type: 'file', filename: threatPath, "maxLogSize": 10485760, "backups": 10 } 
		},
  		categories: { 
			default: { appenders: ['info', 'default'], level: 'info' },
			exception: { appenders: ['exception'], level: 'error' },
			threat: { appenders: ['threat'], level: 'error' } 
		},
		"replaceConsole": true
	})
	// log4js.configure({
...
var initialFirebase = function (config) {
	let fb = firebase.initializeApp(config.firebase.init);
	let fd = fb.database();
	let fa = fb.auth();
	if(config.firebase.auth.email){
		return fa.signInWithEmailAndPassword(config.firebase.auth.email,config.firebase.auth.password)
    	.then(v => {
		    fa.onAuthStateChanged(v => {
				if(!v){
					fa.signInWithEmailAndPassword(config.firebase.auth.email,config.firebase.auth.password);
				}
			})
			return Promise.resolve({fb, fd, fa});
		})
	} else {
		return Promise.resolve({fb, fd, fa});
	}
...
var initialBot = function ({config, logger, db, i18n, fbase}) {
	...
	var createBot = function (botPath) {
		...
		var bot = new Bot();
		return bot.init({config, logger, db, i18n, fbase});
	}
...
.then(config => {
	return Promise.all([
		...
		initialDB(config),
		initialFirebase(config)
	]).then(rs => {
		return Promise.resolve({ config, i18n: rs[0], logger: rs[1], db: rs[2], fbase: rs[3] });
	});
})
.then(initialBot)
.then(startService)
